...
export default function MarkdownEditorPanel({
  ...
  return (
    <div className="border-primary/30 bg-card/95 fixed inset-x-4 bottom-24 z-[90] max-h-[40vh] overflow-auto rounded-md border p-6 shadow-2xl">
        ...
        {Object.entries(draft.frontmatter).map(([key, value]) => (
          <label key={key} className="block space-y-1">
            ...
            <input
              ...
              className="border-border bg-background w-full rounded-sm border px-3 py-2 outline-none"
            />
          </label>
        ))}
      </div>
