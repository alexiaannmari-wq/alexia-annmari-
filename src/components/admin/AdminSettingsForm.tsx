...
const groups: Array<{ title: string; fields: FieldConfig[] }> = [
  ...
  },
  {
    ...
    fields: [
      ...
      { path: 'pages.specialties.label', label: 'Services' },
      { path: 'pages.fees.label', label: 'Packages' },
      { path: 'pages.faq.label', label: 'FAQ' },
      { path: 'pages.book.label', label: 'Intro call page' },
      { path: 'pages.book.consultationLabel', label: 'Intro call tab' },
  ...
  },
  {
    ...
    fields: [
      ...
      {
        path: 'forms.bookSession.externalLink.url',
        label: 'External booking URL (blank hides button)',
      },
      {
...
export default function AdminSettingsForm() {
  ...
  return (
    <div className="space-y-8">
      {groups.map((group) => (
        <section key={group.title} className="bg-card rounded-md border border-border p-8">
      ...
      <section className="bg-card rounded-md border border-border p-8">
        <h2 className="font-heading text-2xl font-bold">Engagement notes & intro call types</h2>
        <label className="mt-6 block space-y-2">
          <span className="text-sm font-bold">Engagement notes heading</span>
          <input
        ...
        <div className="mt-8 space-y-6">
          {sessionTypes.map((sessionType, index) => (
            <div key={`session-${index}`} className="space-y-3 rounded-sm border border-border p-4">
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className="rounded-sm border border-border px-6 py-3 font-bold"
        >
          Discard
        </button>
      </div>
