  ...
  if (admin?.isEditMode) {
    ...
    return (
      <Component
        ...
        className={`border-primary/40 bg-background/95 w-full rounded-sm border px-3 py-2 outline-none ${className ?? ''}`}
      ...
      <div className={className}>
        {value.map((item, index) => (
          <textarea
            ...
            className="border-primary/40 bg-background/95 mb-3 w-full rounded-sm border px-3 py-2 outline-none"
            rows={2}
          />
        ))}
      </div>
  ...
  if (admin?.isEditMode) {
    return (
      <div className="space-y-4">
        {value.map((item, index) => (
          <div key={`${path}-${index}`} className="space-y-2 rounded-sm border border-border p-4">
            <input
              value={item.title}
              onChange={(event) => {
                const next = [...value];
              ...
              className="border-primary/40 bg-background/95 w-full rounded-sm border px-3 py-2 outline-none"
            />
            <textarea
