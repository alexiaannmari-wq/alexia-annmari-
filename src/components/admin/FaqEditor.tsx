  ...
  return (
    <div className="space-y-6">
      {items.map((item, index) => (
        <div key={item.id} className="bg-card space-y-3 rounded-md border border-border p-6">
          <input
            value={item.question}
            onChange={(event) => updateItem(index, { question: event.currentTarget.value })}
            className="border-border bg-background w-full rounded-sm border px-4 py-3 font-bold outline-none"
          />
          <textarea
            value={item.answer}
      ...
      <div className="flex gap-3">
        <button type="button" onClick={addItem} className="rounded-sm border border-border px-5 py-2 font-bold">
          Add FAQ
        </button>
        <button
          type="button"
