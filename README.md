...
| --- | --- | --- |
| Brand name, contact, photos, nav labels, packages, booking copy | `src/content-data/practice.json` | `/admin/settings` |
...
| Service / specialty pages | `src/content/specialties/` | Inline edit mode on specialty pages |
...
The site includes a static admin interface at `/admin` that commits changes directly to GitHub using a personal access token. No backend or database is required.

1. Copy `.env.example` to `.env` and set `PUBLIC_GITHUB_OWNER`, `PUBLIC_GITHUB_REPO`, and `PUBLIC_GITHUB_BRANCH` to match this GitHub repository.
...
3. Visit `/admin`, sign in with your token, and use:
   - **Site Settings** for shared brand details, packages, nav labels, and form copy
   - **FAQ** for question/answer management
   - **Edit mode** on public pages for page-local copy and image replacements

Changes are saved as Git commits to `main`, which triggers the GitHub Pages deploy workflow.

**Security:** the token is stored only in the browser (`localStorage`). Never commit tokens to the repository.
...
Saves go directly to files in the working tree via a dev-only API (`/__admin/content/*`) — no GitHub commits are created. Edit mode and session state persist across page navigation via `localStorage`.

## Brand and contact details

Shared site copy lives in `src/content-data/practice.json` and is editable via `/admin/settings`. Structural settings (page toggles, form backends, captcha, calendar URLs) remain in `src/config/site.structural.ts`.
...
Set `logo` to a full wordmark for the large header at the top of the page. Set `logoMark` to a compact mark for the shrunk sticky header after scrolling (mark only). Leave both blank to show `practiceName` with the default heart icon.
...
3. Keep paths starting with `/images/`, for example `/images/headshot.webp`.
...
## Add or edit service pages

Service pages live in `src/content/specialties/`.

To add a service:

1. Copy an existing file, such as `inbox-calendar.md`.
2. Rename it, for example `travel-ops.md`.
...
description: Reliable day-to-day support for email, scheduling, and follow-through.
order: 1
heroImage: /images/nature-temp.jpg
---
```

The `order` field controls the order on the Services page.
...
## Edit packages and FAQ

Package tiers and engagement notes are in `src/content-data/practice.json`. Booking/service rows come from `forms.bookSession.sessionTypes` (label, `durationMinutes`, fee, and `description`). Engagement notes live under `fees.insuranceNotes`.

FAQ content is in `src/content-data/faq.json`. Each item has an `id`, `question`, and `answer`.

## Connect forms

The Contact and Book forms are controlled in `src/config/site.structural.ts` (backends and providers) and `src/content-data/practice.json` (form copy and labels).
...
Demo mode shows an on-page success message but does not send email. Use it for local preview only.

## Add booking and public calendar availability

The `/book` page can show two booking modes:

- `Book a Consultation` — intro-call request form.
- `Book a Session` — session request form with optional live availability from a public Google Calendar.
...
"externalLink": {
  ...
  "description": "Schedule a short call to discuss fit and priorities."
}
```
...
bookSession: {
  ...
  sessionTypes: [
    ...
    },
    {
      ...
      description: 'Focused time on inbox, calendar, or operations priorities.',
      modes: ['virtual'],
      kind: 'session',
    },
  ],
...
Each `sessionTypes` entry uses `durationMinutes`. Available times are shown using that session length (for example, 9:00 AM – 9:50 AM for a 50-minute session).
...
- recalculates openings when a visitor changes between session lengths
...
Set the ICS feed URL directly, or let the site derive it from the public calendar link:
...
Use those fields in the form backend dashboard, email templates, automations, or serverless endpoint to route submissions.
...
- `basic` - hides the forms and shows an email-only invitation using the address in `practice.json`.
- `demo` - no backend; useful for local preview while editing copy.
...
- `resend` - posts to a secure backend route that sends email through Resend.
- `custom` - posts to any custom endpoint provided.
...
When forms are disabled, the Contact and Book pages still display contact information and ask visitors to email the address in `src/content-data/practice.json`.
...
Resend requires a secure backend or serverless function because API keys must never be exposed in browser code. This repo includes Resend endpoint settings:
...
Create those backend routes on the hosting platform and use the Resend API key there.
...
When captcha is enabled, the site renders the correct widget and loads the provider script. The form backend should verify captcha responses server-side before trusting submissions.

Before using forms with clients, make sure the provider is appropriate for the kind of information collected and the privacy requirements in the relevant region.
...
This is a static Astro site. Production deploys run through GitHub Pages; the same `dist` output also works on Netlify, Vercel, Cloudflare Pages, or any static host.
